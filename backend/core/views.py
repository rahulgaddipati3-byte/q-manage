...
def _is_expired(token) -> bool:
    """
    Safe expiry check.
    - If Token has expires_at, we use it.
    - If no expires_at field exists, treat tokens as NOT expiring.
    """
...
def _mark_expired_if_needed(token):
    if token and token.status == STATUS_ACTIVE and _is_expired(token):
...
def token_status(request, number):
    ...
    try:
        # IMPORTANT: per-day unique token numbers
        token = Token.objects.get(number=number, service_date=today)
...
def consume_token(request):
    ...
    with transaction.atomic():
        try:
            token = Token.objects.select_for_update().get(number=number, service_date=today)
        except Token.DoesNotExist:
            return JsonResponse({"ok": False, "error": "Token not found"}, status=404)
        ...
        try:
            with transaction.atomic():
                ...
                token = Token.objects.create(
                    counter=counter,              # can be null (reception flow)
...
# --------------------------------------------------
# API: Next token (POST)
# FIXED: marks current token USED, then assigns next ACTIVE
# Body: {"counter":"A1"}
# --------------------------------------------------
@csrf_exempt
def next_token(request):
    ...
    with transaction.atomic():
        # 1) If this counter currently has an ACTIVE token, it must become USED when staff calls next
        current = (
            Token.objects.select_for_update()
            .filter(service_date=today, counter=counter, status=STATUS_ACTIVE)
            .order_by("created_at")
            .first()
        )
        if current:
            _mark_expired_if_needed(current)
            if current.status == STATUS_ACTIVE:
                current.status = STATUS_USED
                if hasattr(current, "used_at"):
                    current.used_at = timezone.now()
                    current.save(update_fields=["status", "used_at"])
                else:
                    current.save(update_fields=["status"])

        # 2) Take the next ACTIVE token for today
        # Prefer unassigned (reception-issued), else any other active token not already assigned here
        token = (
            Token.objects.select_for_update()
        ...
        if not token:
            token = (
                Token.objects.select_for_update()
                .filter(service_date=today, status=STATUS_ACTIVE)
                .exclude(counter=counter)  # not already on this counter
                .order_by("sequence")
                .first()
            )

        # Expire skip (if expiry exists)
        ...
        # 3) Assign selected token to this counter (this becomes "now serving")
...
# FIXED: scope to today, show next ACTIVE token
# --------------------------------------------------
@require_GET
def queue_status(request):
    ...
    # waiting = active tokens for today only
    unassigned = Token.objects.filter(service_date=today, status=STATUS_ACTIVE, counter__isnull=True)
    assigned_other = Token.objects.filter(service_date=today, status=STATUS_ACTIVE).exclude(counter=counter)

    next_tok = (
        unassigned.order_by("sequence").first()
        or assigned_other.order_by("sequence").first()
    )

    return JsonResponse({
        "ok": True,
        "counter": counter.code,
        "waiting_total": unassigned.count() + assigned_other.count(),
